...
Loading libraries used throughout this notebook.
```{r}
library(ggplot2)
library(dplyr)
set.seed(123)
...
A: We have trained a GLM model on a small dataset of 50 instances. Then we have evaluated the 'true risk' using a huge dataset. After this we have evaluated the estimated risk of this model using other 50 instances datasets. First, on average the estimated risk on 50 instances is a good approximation on the true risk since the average difference is really small. But the variance (standard error) of these estimates is big. We see that these estimates vary quite a lot based on the testing data but by computing the 95% confidence intervals we can capture the true risk approximately 93% of the time.  
...
A: Surely the true empirical risk decreases by increasing the training data,this experiment proves that. If we would to increase more data the difference between the risks would be smaller since the model h1 will see enough data to learn the DGP and even though h2 has more data they both will have learned the DGP well, hence the smaller difference.
...
Then we used holdout estimation (multiple times) **on the same dataset** and trained on 50%, then tested on 50% and said okay this will be the error on of the model when trained on all 100% data. Now from the experiment we saw that this estimate is biased since the loss will be in reality SMALLER. The median standard error is also big due to the randomness of the train-test split. The standard error even suggests that we won't always overestimate the risk, sometimes we will even underestimate it.  
...
A: Compared to the normal train-test split we observe how the bias and variability decreases. LOOCV has the lowest bias and low median standard error. Next is 10-CV which has second lowest bias and finally the 4-CV with the largest median standard error. 
```{r}
